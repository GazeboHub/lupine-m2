...
(declaim (inline hex-digit-char first-or-self listify xmemq))
(defun hex-digit-char (code) (digit-char code 16))
...
(defun class-all-subclasses (class)
 ;;; Collect unsorted list of all descending subclasses of the class given.
  ;; (Similar to LW-TOOLS::CLASS-ALL-SUBCLASSES, but returns metaobjects instead of names)
  ;; The result is a freshly-consed list not including the class itself.
  (let ((acc ()))
    (labels ((%collect (list)
               (dolist (class list)
                 (pushnew class acc)
                 (%collect (#+lispworks clos:class-direct-subclasses
  ...
  (let ((length (length vector))
    ...
    (case bit-count
      ((8 nil)				; (vector (unsigned-byte 8))
       (setq string (make-string (if char 
                                     (+ #1=(ash length 1)
                                        (if (< 0 length) (truncate (1- length)interval) 0))
                                     #1#)
...
(defun interleave-seq (result-type delimiter seq)
  (unless (every #'null seq)
    (case result-type
      (string
       (with-output-to-string (stream)
         (do* ((rest seq (rest rest))
...
Arguments:
 FORM
   ...
   macros or special operators in Lisp. For now, the following are recognized:
     IF WHEN UNLESS AND OR NOT QUOTE PROGN SETQ CASE.

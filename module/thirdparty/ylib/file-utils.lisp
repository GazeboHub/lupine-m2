...
If OVERWRITE is true overwrites the file designtated by TO if it exists."
  ...
  #-:allegro
  (let ((element-type #-:cormanlisp 'octet
...
(defun get-folder (what &key create)
 ;;; Workaround to identify Windows special folders as pathnames, not as files
  ...
  ;;     Windows Server 2008: #P"C:/Users/user/AppData/Roaming/"
  ;; :common-appdata
  ;;	Detection of the all users path requires IE 5 and above.
  ;;	Not available on Windows 95 with IE 4 without Active Desktop istalled.
  ;; :local-appdata
  ...
  ;;     Windows Server 2008: #P"C:/Users/user/AppData/Local/"
  ;; :documents (= :my-documents), :common-documents
 ...
 ;;; CAUTION: LWW probe-file can result in device and version eql to :UNSPECIFIC
  ...
  (cond ((and (symbolp arg1) (symbolp arg2))			; works for NILs
        ...
        ((and (stringp arg1) (stringp arg2))
         (#-linux locale-string-equal #+linux string= arg1 arg2))
...
(defun get-file-title (pathname)
  ...
  (fli:with-dynamic-foreign-objects ()
    (fli:with-foreign-string (lpszFile element-count byte-count
                                       :external-format *internal-format*)
        (namestring pathname)
      (declare (ignore element-count byte-count))
      (let ((buffer (fli:allocate-dynamic-foreign-object :type :wchar-t :nelems 255)))
        (if (zerop (|GetFileTitle| lpszFile buffer 255))	; success
